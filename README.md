# 간단한 게시판 사이트
서비스 링크: https://simple-board-b06t.onrender.com/ <br>
백엔드 실습용으로 만든 게시판 사이트입니다.<br>
...
## 로컬 실행법
배포글과 대댓글을 작성할 수 있고 삭제할 수 있습니다.<br>
어드민이 아닌 유저는 본인의 댓글/대댓글만 삭제할 수 있습니다.<br>
댓글 삭제시, 대댓글이 없는 댓글이나 대댓글은 바로 삭제되며, 대댓글이 있는 댓글의 경우 대댓글은 남아 있습니다.<br>
메인 화면에서 게시글의 댓글 수를 확인할 수 있습니다.<br>
* **하트**<br>
...
## 로컬 실행법
배포를 했고 실행이 되는 걸 확인했으나 혹시나 접속이 안될 것을 대비하여 로컬 실행법을 첨부합니다.<br>
**첫 접속시 로딩이 오래 걸릴 때에는 에러가 뜬 게 아니라면 잠시 기다려주세요!!** <br>
1. local 브랜치를 clone합니다.
2. postgreSQL이 설치된 상태에서 SQL Shell에 ```\c simple_board```를 입력해서 db를 만듭니다.
3. 루트 디렉토리에 다음 내용의 .env 파일을 만듭니다.
```
PORT=3000
HOST=localhost
DB_USER=[이 글을 읽는 사람의 유저이름]
DB_NAME=simple_board
DB_PASSWORD=[이 글을 읽는 사람의 패스워드]
DB_PORT=5432
COOKIE_SECRET=[아무거나 입력]
```
4. 터미널에 ```npm install```과 ```npm start```를 차례로 입력합니다.
